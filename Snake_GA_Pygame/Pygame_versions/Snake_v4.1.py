...
class SnakeAI:
    ...
    def move(self):
        ...
        if new_distance < old_distance:
            self.score += 1  # Reward for moving toward food
        else:
            self.score -= 1  # Penalize moving away from food
        
        if new_head == self.food:
            self.score += 20  # Stronger reward for food
            self.length += 1  # Increase length counter
            self.food = self.spawn_food()
        else:
            self.snake.pop()
        
        self.score += 0.5 + (self.length * 0.05)  # Longer survival gets better rewards
