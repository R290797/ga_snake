...
PRETRAINED_MODELS = {
    "Model 1": [0.075, 0.298, 0.348, 0.123, -0.309, -0.390, -0.216, 0.526, -0.289],
    "Model 2": [0.089, 0.310, 0.300, 0.100, -0.250, -0.350, -0.200, 0.500, -0.270],
    "Model 3": [1.157, 0.905, 0.270, 0.270, -1.433, -2.066, -1.556, 3.811, -1.451],
    "Model 4": [1.225, 1.015, 0.250, 0.514, -0.840, -2.533, -1.245, 2.521, -2.005]
}
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if replay_button.collidepoint(event.pos):
                    return mode  # **Return the mode to determine next action**
...
def run_manual_mode():
    while True:  # Allow replaying the game
        ...
        if action == "manual":
            continue  # Restart the manual mode
        else:
            break  # Quit game
...
def show_pretrained_models():
    ...
    title_text = font_large.render("Select Pre-Trained Model", True, BLACK)
    ...
    model_explanations = {
        "Model 1": "50 Snakes - 10 generations.",
        "Model 2": "50 Snakes - 50 generations.",
        "Model 3": "50 Snakes - 100 generations.",
        "Model 4": "50 Snakes - 200 generations."
    }
...
def show_training_summary(best_score, best_length, training_time):
    ...
    # **Render Button Text**
    replay_text = font_small.render("Train", True, WHITE)
    quit_text = font_small.render("Quit", True, WHITE)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if menu_button.collidepoint(event.pos):
                    ...
                    return "replay"  # Restart training
                elif quit_button.collidepoint(event.pos):
                    pygame.quit()
                    sys.exit()
...
def run_pretrained_ai(model_params):
    ...
    while running and snake.alive:
        ...
        draw_game()  # Ensure the game is redrawn
