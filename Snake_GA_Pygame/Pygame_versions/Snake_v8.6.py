...
class SnakeAI:
    ...
    def choose_direction(self):
        def simulate_move(head, direction, depth=0):
            ...
            loop_penalty = self.brain[3] * (-15 * visit_count if visit_count > 1 else 0)
    ...
    def move(self):
        ...
        if self.detect_loop():
            self.fitness_score -= 50  # Penalize fitness score instead of instant death
            self.alive = False  

        if time.time() - self.last_food_time > 10:
            self.alive = False  # No food for 10 seconds = instant death
...
def evolve_snakes(snakes):
    ...
    top_performers = snakes[:5]  # Select the top 5fittest snakes
