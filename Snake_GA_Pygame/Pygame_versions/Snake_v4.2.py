...
class SnakeAI:
    ...
    def detect_loop(self):
        """Detects if the snake is stuck in a loop by checking repeated positions."""
    ...
    def choose_direction(self):
        """Determines the best movement direction based on food distance."""
    ...
    def move(self):
        """Moves the snake in the chosen direction and updates the game state."""
        if not self.alive:
            return
        ...
        if self.detect_loop():
            self.score -= 5  # Stronger penalty for looping
        
        if new_head == self.food:
            self.score += 20  # Reward for eating food
            self.length += 1  # Track food consumption
            self.food = self.spawn_food()
        else:
            self.snake.pop()
        
        self.score += 0.5 + (self.length * 0.05)  # Reward for survival
        
        if head_x <= GAME_AREA_X + CELL_SIZE or head_x >= GAME_AREA_X + GAME_AREA_WIDTH - CELL_SIZE or \
           head_y <= GAME_AREA_Y + CELL_SIZE or head_y >= GAME_AREA_Y + GAME_AREA_HEIGHT - CELL_SIZE:
            self.score -= 0.5  # Small penalty for staying near walls
...
def draw_game():
    ...
    score_text = font.render(f"Best Score: {max(snake.score for snake in snakes)}, Length: {max(snake.length for snake in snakes)}, Time: {elapsed_time}s", True, WHITE)
...
def run_generation():
    ...
    generation_start_time = time.time()  # Reset time at the start of each generation
    running = True
    ...
    top_performers = snakes[:10]  # Select best performers
    new_population = [SnakeAI() for _ in range(population_size - 1)]
