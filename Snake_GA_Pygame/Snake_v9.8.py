...
class SnakeAI:
    def __init__(self, brain=None):
        ...
        # AI Weights
        if brain is None:
            self.brain = np.random.uniform(-0.75, 0.75, 9)  # Smaller range for stability
    ...
    def fitness_function(self):
        ...
        fitness = (self.score * self.length * 5) + (time.time() - self.start_time) * 10 - (self.moves_made / (self.score + 1))
...
def draw_game(game_mode="train_ai", model_params=None):
    ...
    if game_mode == "train_ai":
        ...
        small_font = pygame.font.SysFont(None, 24)  # Reduce font size for better fit
        ...
        for i, (label, value) in enumerate(zip(param_labels, model_params)):
            param_value_text = small_font.render(f"{label}: {value:.2f}", True, GREEN)
            screen.blit(param_value_text, (text_x, text_y + (i + 1) * 22))  # Reduce spacing for better fit
...
def show_game_over_screen(snake, mode="manual"):
    ...
    # **New Menu Button**
    menu_button = pygame.Rect(WIDTH // 2 - button_width * 1.5 - button_spacing, button_y, button_width, button_height)
    replay_button = pygame.Rect(WIDTH // 2 - button_width // 2, button_y, button_width, button_height)
    quit_button = pygame.Rect(WIDTH // 2 + button_width // 2 + button_spacing, button_y, button_width, button_height)
...
def run_manual_mode():
    while True:  # Allow replaying the game
        ...
        action = show_game_over_screen(snake, mode="manual")
        if action == "manual":
            continue  # Restart manual mode
        elif action == "menu":
...
def show_pretrained_models():
    ...
    while True:
        ...
        for button_rect, model_name, params in model_buttons:
            ...
            explanation_text = font_small.render(model_explanations[model_name], True, BLACK)
            screen.blit(explanation_text, (button_rect.x + button_width + 20, button_rect.y + 10))
...
def show_training_summary(best_score, best_length, training_time):
    ...
    for i, text in enumerate(stats_text):
        stat_render = font_small.render(text, True, BLACK)
        screen.blit(stat_render, (WIDTH // 2 - stat_render.get_width() // 2, stats_start_y + i * 40))
    ...
    pretrain_button = pygame.Rect(start_x + button_width + button_spacing, row1_y, button_width, button_height)
    ...
    quit_button = pygame.Rect(start_x + button_width + button_spacing, row2_y, button_width, button_height)
    ...
    pretrain_button = pygame.Rect(start_x + button_width + 20, button_y, button_width, button_height)
    replay_button = pygame.Rect(start_x + (button_width + 20) * 2, button_y, button_width, button_height)
    quit_button = pygame.Rect(start_x + (button_width + 20) * 3, button_y, button_width, button_height)
...
def run_pretrained_ai(model_params):
    ...
    if action == "model_selection":
        model_name, model_params = show_pretrained_models()  # Return to Pre-Trained Model Selection
