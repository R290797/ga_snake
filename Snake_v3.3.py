...
class SnakeAI:
    def __init__(self):
        ...
        self.alive = True
        self.brain = SnakeNet()
        self.steps_without_progress = 0  # Track steps with no improvement
    ...
    def move(self):
        ...
        if new_distance < old_distance:
            self.score += 2  # Higher reward if closer
            self.steps_without_progress = 0  # Reset counter when progress is made
        else:
            self.score -= 1  # Stronger penalty if moving away
            self.steps_without_progress += 1  # Track no progress

        if new_head == self.food:
            self.score += 10
            self.food = self.spawn_food()
            self.steps_without_progress = 0  # Reset loop counter when food is eaten
        else:
            self.snake.pop()
        
        self.score += 0.5  # Reward for survival
        
        # Penalize being stuck in a loop
        if self.steps_without_progress > 15:
            self.alive = False  # Kill the snake if it's not making progress
...
population_size = 50  # Reduced to improve visualization
...
def run_generation():
    ...
    top_performers = snakes[:2]  # Keep the top 2 snakes unchanged
    new_population = []
    for _ in range(population_size - 1):  # Keep one elite
        ...
        with torch.no_grad():
            for param in offspring.brain.parameters():
                param += torch.randn_like(param) * 0.3  # Stronger mutation for more exploration
