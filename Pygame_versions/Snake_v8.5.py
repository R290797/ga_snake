...
class SnakeAI:
    ...
    def detect_loop(self):
        ...
        loop_length = 10  # The typical size of a looping pattern
        history_window = 20  # The number of recent moves to analyze
        ...
        # Check if any position is repeated excessively
        max_repeats = max(position_counts.values())

        # If a position is visited 4 times within 20 moves, it suggests looping
        if max_repeats >= 4:
            return True
...
def evolve_snakes(snakes):
    ...
    top_performers = snakes[:5]  # Select the top 10 fittest snakes
    best_snake = top_performers[0]
    new_snakes = []
    ...
    for _ in range(len(snakes)):  # Maintain same population size
        ...
        mutation = np.random.randn(len(new_brain)) * 0.2  
        new_brain += mutation
...
snakes = [SnakeAI() for _ in range(20)]
