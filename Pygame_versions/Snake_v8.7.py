...
class SnakeAI:
    ...
    def fitness_function(self):
        ...
        fitness = (self.score * self.length * 2) + (time.time() - self.start_time) * 5
...
def draw_game():
    ...
    avg_length_text = font.render(f"Avg Length: {avg_length:.2f}", True, WHITE)

    # **Position Each Text in its Section**
    screen.blit(score_text, (section_width * 0.1, 10))  # First section
    screen.blit(time_text, (section_width * 1.1, 10))   # Second section
    screen.blit(length_text, (section_width * 2.1, 10)) # Third section
    screen.blit(avg_length_text, (section_width * 3.1, 10))   # Fifth section
...
def evolve_snakes(snakes):
    ...
    for _ in range(len(snakes)):  # Maintain same population size
        parent1 = random.choice(top_performers[:3]) 
        parent2 = random.choice(top_performers)
        parent3 = winner_snake

        # **Crossover: Mix weights from two parents**
        new_brain = (parent1.brain + parent2.brain + parent3.brain
                     ) / 3  
